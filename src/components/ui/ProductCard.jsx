...
export default function ProductCard({ product: p }) {
  ...
  return (
    ...
    >
      {/* Image area */}
      <div style={{ background: p.bg_color, height: 180, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', flexShrink: 0 }}>
        <motion.div whileHover={{ scale: 1.08 }} transition={{ duration: 0.3 }} style={{ fontSize: 60, filter: 'drop-shadow(0 4px 12px rgba(0,0,0,0.1))' }}>
          {p.emoji}
        </motion.div>

        <button onClick={handleWish} style={{ position: 'absolute', top: 10, right: 10, background: 'white', border: 'none', borderRadius: '50%', width: 30, height: 30, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
