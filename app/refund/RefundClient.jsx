...
export default function RefundClient() {
  ...
  async function requestRefund() {
    ...
    try {
      ...
      const { error: err } = await supabase.from('refunds').insert(payload)
      if (err) throw err
      setReason('')
      // refresh list
      fetchRefunds()
          ...
          <section style={{ marginTop: 28 }}>
            ...
            {!user && (
              <div style={{ color: C.muted, marginBottom: 12 }}>
                Please <a href="/account" style={{ color: C.forest }}>sign in</a> to view or
                request refunds.
              </div>
            )}
            ...
            {user && refunds.length > 0 && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 8 }}>
                {refunds.map((r) => (
                  <div
                    key={r.id}
                    style={{ background: C.card, padding: 12, borderRadius: 10, border: `1px solid ${C.border}` }}
                  >
          ...
          <section style={{ marginTop: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxWidth: 560 }}>
              <textarea
                ...
                rows={4}
                style={{ padding: 12, borderRadius: 10, border: `1px solid ${C.border}`, fontFamily: 'inherit' }}
              />
              <div style={{ display: 'flex', gap: 10 }}>
                <button
                  onClick={requestRefund}
                  disabled={loading || !user}
                  style={{ background: C.forest, color: 'white', border: 'none', borderRadius: 10, padding: '10px 16px', cursor: 'pointer', fontFamily: 'inherit' }}
                >
                  {loading ? 'Submitting…' : 'Submit Refund Request'}
                </button>
                <button
                  onClick={() => { setReason('') }}
                  style={{ background: 'none', border: '1px solid ' + C.border, borderRadius: 10, padding: '10px 16px', cursor: 'pointer', fontFamily: 'inherit' }}
                >
                  Clear
                </button>
              </div>
